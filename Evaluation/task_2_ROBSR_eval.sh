...
# Get the current date and time for file logging
current_time=$(TZ="America/Los_Angeles" date +"%Y-%m-%d_%H-%M-%S")
pid_file="logs/PIDs/PIDs_from_task_2_ROBSR_eval_${exp_name}_${current_time}.pids"
...
python3 pre_process/create_task_2_prompt.py --dataset "$dataset_path" --prompt_template_name "$prompt_template_name" --prompt_dict_name "$exp_name" --limit_k ${limits}
...
if [[ "${regeneration}" == "True" ]]; then
    ...
    python3 post_process/task_2_parsing_second_round.py --model_name ${model_name} --model_output_path "generation/outputs/${exp_name}_regen/collected_results.pickle" --parsed_output_path "post_process/parsed_output/${exp_name}.pickle" --original_prompt_dict_path "generation/prompts/${exp_name}.pickle"
...
python3 post_process/task_2_evaluate.py --model_name ${model_name} --input_path "./post_process/parsed_output/${exp_name}.pickle" --limit_k ${limits} --dataset_path ${dataset_path} --exp_name ${exp_name} --logging_csv "post_process/logs.csv"
